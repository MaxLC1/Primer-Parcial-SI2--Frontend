...
export class VestidorVirtual implements OnInit {
  ...
  ngOnInit() {
    this.http.get<any[]>('http://34.230.18.9:8000/api/v1/catalogo/productos').subscribe(data => {
      this.productos = data;
  ...
  async probarPrenda() {
    ...
    try {
      ...
      const proxyResponse = await this.http.get<any>(`http://34.230.18.9:8000/api/v1/archivos/proxy-imagen?url=${encodeURIComponent(rawUrl)}`).toPromise();
  ...
  getImagenUrl(url: string | null): string {
    ...
    if (url.startsWith('http://192.168.')) {
      return url.replace(/http:\/\/192\.168\.\d+\.\d+:\d+\//, 'http://34.230.18.9:8000/');
    }
    if (!url.startsWith('http')) {
      return 'http://34.230.18.9:8000/' + url;
    }
    return url;
  }
}

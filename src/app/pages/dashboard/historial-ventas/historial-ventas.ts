...
export class HistorialVentas implements OnInit {
  ...
  loadMetadata() {
    ...
    this.http.get<any[]>('http://127.0.0.1:8000/api/v1/sucursales/sucursales').subscribe(res => { this.sucursales = res; this.checkLoading(); });
    this.http.get<any[]>('http://127.0.0.1:8000/api/v1/seguridad/usuarios').subscribe(res => { this.usuarios = res; this.checkLoading(); });
